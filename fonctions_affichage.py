...
def plot_graphs_multi(result, semaine=5) :
    ...
    for i in range(N):
        # PHS Level
        axes[0, i].plot(result['phs_level'][:T_plot, i], color='blue')
        if i == 0: axes[0, i].set_ylabel("PHS stock (MW)")
        axes[0, i].set_title(f"{countries[i]}")
        ...
        # Déficit (énergie non servie)
        axes[2, i].plot(result['deficit_final'][:T_plot, i], color='red') # 'deficit' pour la v3 sur 1 an ?
        if i == 0: axes[2, i].set_ylabel("Deficit (MW)")
        ...
        # Flux entrants
        axes[4, i].plot(received[:T_plot, i], color='purple')
        if i == 0: axes[4, i].set_ylabel("Imports (MW)")

        # Surplus (perdu ou non utilisé)
        axes[5, i].plot(result['surplus_final'][:T_plot, i], color='gray') # 'surplus' dans la v3 1 an
        if i == 0: axes[5, i].set_ylabel("Surplus (MW)")
        axes[5, i].set_xlabel("Time (Hours)")

    plt.suptitle(f"Results for {T_plot} hours", fontsize=16)
    plt.tight_layout()
    plt.show()
    ...
    for i in range(N):
        ...
        axes[5, i].plot(result['surplus_final'][T0:T1, i], color='gray')
        if i == 0: axes[5, i].set_ylabel("Surplus (MW)")
        axes[5, i].set_xlabel("Hours week")

    plt.suptitle(f"Zoom — Week {semaine}", fontsize=16)
    plt.tight_layout()
    plt.show()
...
def plot_global_graph_multi(result, demand_N, wind_N, wind_cap_N, solar_N, solar_cap_N, ptg_eff, phs_eff, semaine=5) :
    # ===============================
    # Flux échanges
    # ===============================
...
def plot_energy_balance(
    ...
    for i in range(N):
        ...
        if show_surplus:
            surplus_i = -result["surplus before exchange"][T0:T1, i]
        ...
        if show_max_storage:
            axes[i].plot(time_axis, -max_storage_phs, color='red', linewidth=2, linestyle='--', label='Max capa PHS')
            axes[i].plot(time_axis, -max_storage_ptg, color='green', linewidth=2, linestyle='--', label='Max capa PtG')
        ...
        if i == 0:
            ...
            if show_max_storage:
                legend_labels.extend(['Max capa PHS', 'Max capa PtG'])
